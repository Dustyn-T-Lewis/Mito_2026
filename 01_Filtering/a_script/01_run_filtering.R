...
clear_dir(data_dir); clear_dir(report_dir)

#### Load matrix + metadata ####
# Read the DIA-NN protein matrix, split annotation from intensities, attach the sample sheet.
...
# The intersection drops abundant-cellular bovine proteins (GAPDH/actin/tubulin) that
# are real H9c2 biology and keeps only true plasma proteins.
...
#### Dedup + build DAList ####
# Collapse any duplicate accessions, then package matrix/annotation/metadata into the proteoDA object.
...
#### Export ####
# Write the filtered DAList for Stage 02, plus an audit workbook and a contaminant bar plot.
...
write.xlsx(
  ...
  file.path(data_dir, "filtering_report.xlsx"), overwrite = TRUE)
