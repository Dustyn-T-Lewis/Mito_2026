...
source(here::here("04_Figures", "functions", "shared_composite_layout.R"))
source(here::here("04_Figures", "functions", "shared_data_loaders.R"))
source(here::here("04_Figures", "functions", "shared_enrichment_ora.R"))
source(here::here("04_Figures", "functions", "f01_mitocarta_lens.R"))
...
pca_p <- pca_res$plot
dep_dat <- dep_data()
dep_p <- build_dep_count_panel(dep_dat)
eff_p <- build_dep_effect_panel(dep_dat)
...
fig <- pca + dep + eff + venn_gg + strip + pw +
  ...
  composite_caption(paste0(
    "Protein DEP significance: Π-score (Xiao 2014, P.Value^|log2FC|) < 0.05 (panels B, D, E). ",
    "Pathway significance: fgsea BH-FDR < 0.05 (panel F)."
  ))
...
sheet_specs <- c(
  list(
    ...
    list(
      name = "dep_counts", df = dep_dat$counts |> select(contrast, threshold, n, pct),
...
mirror_to_box(composite, "02_Figures/F01_Proteome_Overview", guard = TRUE)
mirror_to_box(file.path(DAT, "F01_supplementary.xlsx"), "03_Supplementary", guard = TRUE)
