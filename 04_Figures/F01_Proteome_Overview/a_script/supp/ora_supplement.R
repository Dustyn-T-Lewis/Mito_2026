...
shown <- bind_rows(lapply(unname(CONTRASTS), function(cn) {
  ...
  mutate(
    ...
    fdr = paste0(
      ifelse(padj < 0.05, "*", ""),
      ifelse(padj < 0.001, formatC(padj, format = "e", digits = 0), formatC(padj, format = "f", digits = 3))
    )
  ) |>
  arrange(contrast, lp) |>
...
sub <- "Disease / Transplant / Rescue; over-representation on the 5-DB pool, EnrichmentMap-deduplicated; FDR < 0.10 (* = FDR < 0.05); left = down, right = up; bar = -log10 FDR; colour = database"
...
mirror_to_box(file.path(PNG, "SUPP_F01_ora.png"), "02_Figures/F01_Proteome_Overview/supp", guard = TRUE)
