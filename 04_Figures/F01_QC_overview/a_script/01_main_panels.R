...
comb <- load_combined_wide(); dep_df <- comb
...
dep_results <- setNames(lapply(CORE, \(c) as.data.frame(dplyr::filter(comb_long, contrast == c))), CORE)
...
source(here::here("04_Figures", "F01_QC_overview", "a_script", "_panel_lollipop.R"))  # -> pLOL (tag B)
...
source(here::here("04_Figures", "F01_QC_overview", "a_script", "_panel_F_enrichment.R"))  # -> pF (tag E)
source(here::here("04_Figures", "F01_QC_overview", "a_script", "_panel_G_rank.R"))         # -> pG (tag F)
