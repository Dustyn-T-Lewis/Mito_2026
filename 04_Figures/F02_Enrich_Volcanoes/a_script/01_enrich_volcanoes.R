...
# b_reports/panels; the four-ring composite to b_reports/.
...
source(here::here("04_Figures", "functions", "shared_workbook.R"))

BASE <- here::here("04_Figures", "F02_Enrich_Volcanoes")
RPT <- file.path(BASE, "b_reports")
PANELS <- file.path(BASE, "b_reports", "panels")
DAT <- file.path(BASE, "c_data")
for (d in c(RPT, PANELS, DAT)) dir.create(d, recursive = TRUE, showWarnings = FALSE)
...
prep <- lapply(seq_len(nrow(contrasts)), function(i) {
  ...
  dedup <- dedup_report(sig, set_pool)
  enrich_deduped <- deduplicate_enrichment(sig, set_pool) |> arrange(padj)
...
fig <- grid$plot &
  ggplot2::theme(
    ...
    plot.margin = ggplot2::margin(1, 5, 1, 5, "mm")
  )
...
fig <- fig + patchwork::plot_annotation(
  caption = paste0(
    ...
    "Ring arcs: fgsea BH-FDR < 0.05, EnrichmentMap-deduplicated (5-DB lens); arc height = −log10 FDR, colour = NES."
  ),
  theme = ggplot2::theme(
    plot.caption = ggplot2::element_text(size = 8, hjust = 0.5, colour = "grey35")
  )
)
ggplot2::ggsave(
  file.path(RPT, "MAIN_F02_enrich_volcanoes.png"), fig,
...
excluded_sheet <- list(
  name = "excluded", df = as.data.frame(excluded_pathway_log()),
  role = "Sets removed from the 5-DB lens by the fixed name filters",
  contents = paste(
    "pathway, database, n_genes, reason (MitoCarta localisation aggregate or",
    "disease/viral name filter); name-based exclusion applied before ORA/GSEA"
  )
)
build_workbook(
...
mirror_to_box(file.path(RPT, "MAIN_F02_enrich_volcanoes.png"), "02_Figures/F02_Enrich_Volcanoes", guard = TRUE)
mirror_to_box(file.path(DAT, "F02_supplementary.xlsx"), "03_Supplementary", guard = TRUE)
