#!/usr/bin/env Rscript
# Module-level inference on the WGCNA modules, on the same
...
# DiseaseAfter and Recovery complete the orthogonal-pair framework: only D⊥A, T⊥R and
# V⊥I are clean select-then-test pairs, so a module reversal read needs both partners.
CONTRASTS <- c(
  Disease = "PHE - Ctl",
  Transplant = "Mito - Ctl",
  Rescue = "PHE_Mito - PHE",
  DiseaseAfter = "PHE_Mito - Mito",
  Recovery = "PHE_Mito - Ctl",
  Interaction = "(PHE_Mito - PHE) - (Mito - Ctl)"
)

dat <- here("04_Figures", "F03_WGCNA", "c_data")
