...
combined_similarity <- function(genes_a, genes_b) {
...
deduplicate_enrichment_flat <- function(results, pathways, cutoff = 0.375) {
  ...
  for (i in seq_len(nrow(results))) {
    ...
    for (kept in kept_sets) {
      if (combined_similarity(pw_genes, kept) >= cutoff) {
        redundant <- TRUE
        break
      }
    }
...
dedup_report <- function(results, pathways, cutoff = 0.375, cross_db = TRUE) {
  ...
  for (p in redundant) {
    ...
    sims <- vapply(kept, function(b) {
      combined_similarity(pathways[[p]], pathways[[b]])
    }, numeric(1))
    best <- which.max(sims)
...
significant_pathways <- function(fgsea_df, ctr, padj_cut = H9C2_FDR_STD, min_size = 10, max_size = 500) {
  fgsea_df |>
    dplyr::filter(
...
# Audit log of the sets the fixed name filters remove from the canonical
# collection: MITO_DROP_SETS (MitoCarta localisation aggregates) and
# DISEASE_VIRAL_RE (pathogen/disease terms). Contrast-independent and
# deterministic; written to the F02 supplement so the name-based exclusion is
# visible rather than silent.
excluded_pathway_log <- function(
  cache = here::here("04_Figures", "shared", "c_data", "rat_gene_sets.rds")
) {
  gs <- readRDS(cache)
  pw <- do.call(c, unname(gs[CANONICAL_DBS]))
  ...
  reason <- dplyr::case_when(
    nm %in% MITO_DROP_SETS ~ "MitoCarta localisation aggregate",
    grepl(DISEASE_VIRAL_RE, nm, ignore.case = TRUE) ~ "disease/viral name filter"
  )
  keep <- !is.na(reason)
  tibble::tibble(
    pathway = nm[keep], database = classify_database(nm[keep]),
    n_genes = vapply(pw[keep], length, integer(1)), reason = reason[keep]
  ) |>
    dplyr::arrange(reason, pathway)
}
