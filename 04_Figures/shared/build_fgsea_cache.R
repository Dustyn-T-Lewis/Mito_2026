...
#   Input : 03_DEP/a_non_imputed/c_data/combined_results_pi.csv, shared/rat_gene_sets.rds
...
DEP  <- read_csv(here::here("03_DEP", "a_non_imputed", "c_data", "combined_results_pi.csv"),
  ...
  pivot_wider(id_cols = c(uniprot_id, gene), names_from = contrast,
...
write_csv(out, OUT)
cat(sprintf("Saved %s: %d rows, %d pathways, %d databases\n",
