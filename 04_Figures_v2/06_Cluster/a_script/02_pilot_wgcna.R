...
if (file.exists(WGCNA_RDS)) {
  ...
  if (sum(int_keep) >= 4) {
    int_rows <- lapply(colnames(MEs), function(mod) {
      ...
      tibble(
        module = mod, contrast = "Interaction",
  ...
  signs <- me_corr |>
    pivot_wider(
      id_cols = module, names_from = contrast, values_from = r,
      names_prefix = "r_"
    ) |>
  ...
  pal <- setNames(
    viridis::turbo(length(mod_order) + 2)[seq_along(mod_order) + 1],
    mod_order
  )
  ...
  rows <- lapply(mod_order, function(mod) {
    ...
    hdr <- sprintf(
      "Module %s  |  n = %d  |  %s  |  r(D)=%.2f r(R)=%.2f r(T)=%.2f",
  ...
  me_hm <- ggplot(me_corr, aes(.data$contrast, .data$module, fill = .data$r)) +
    ...
    labs(
      title = "F06 pilot_wgcna — module eigengene × contrast indicator",
      x = NULL, y = NULL
    ) +
    FIG_THEME +
...
if (!is.null(results$pilot_wgcna)) {
  ...
  sheet_specs <- c(
    sheet_specs,
    list(list(
      name = "pilot_wgcna_membership", df = wg$pilot_wgcna_membership,
      role = "Gene -> WGCNA module assignment (grey excluded)",
      contents = "gene, module (color label from F05 build)"
    )),
    list(list(
      name = "pilot_wgcna_me_traits", df = wg$pilot_wgcna_me_traits,
    ...
    list(list(
      name = "pilot_wgcna_ora", df = wg$pilot_wgcna_ora,
      role = "Hallmark ORA per module",
      contents = "module, pathway, padj, overlap, size, odds_ratio"
    )),
    list(list(
      name = "pilot_wgcna_ora_mito", df = wg$pilot_wgcna_ora_mito,
