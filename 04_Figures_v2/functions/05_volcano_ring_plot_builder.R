# 04_Figures_v2/functions/05_volcano_ring_plot_builder.R
# Volcano-in-ring composite plot builder. Standard Cartesian ggplot with
# ggforce::geom_arc_bar(); NO coord_polar(). Public entry make_volcano_ring();
# build_ring_180_split() lays out the arc ring and build_nes_legend_bar() draws
# the shared NES colourbar. Consumed by F05 (05_Enrich_Volcano) and C2 (07).
...
build_ring_180_split <- function(top_terms, contrast_name, go_df,
  ...
  scale_arc_height <- function(df) {
    ...
    if (nrow(df) == 1) {
      return(5.6 + (min_height + max_height) / 2)
    }
    neg_lp <- -log10(pmax(df$padj, .Machine$double.xmin))
    rng <- range(neg_lp)
    if (diff(rng) <= 0) {
      return(rep(5.6 + (min_height + max_height) / 2, nrow(df)))
    }
