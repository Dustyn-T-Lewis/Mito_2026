#!/usr/bin/env Rscript
# F04 Panel A: YvO-style ORA composite — central reversal scatter (Disease_Phe
# CTLvPHE vs Rescue_Mito+Phe PHEvPHE_MITO) flanked by 4 corner ORA half-bar
...
COMP_RED  <- unname(DIR_COLORS["Up"])    # exacerbated (same-direction)
COMP_BLUE <- unname(DIR_COLORS["Down"])  # reversed (opposite-direction)
N_SHOW    <- 5L
...
scatter_df <- dep_df |>
  dplyr::transmute(
    gene,
    logFC_x = logFC_CTLvPHE,      logFC_y = logFC_PHEvPHE_MITO,
    pi_Dis  = pi_score_CTLvPHE,   pi_Re   = pi_score_PHEvPHE_MITO) |>
  ...
  dplyr::mutate(
    ...
    sig_class = classify_proteins_f3(pi_Dis, pi_Re),
    is_sig    = sig_class != "NS",
    dist      = sqrt(logFC_x^2 + logFC_y^2),
    quadrant  = dplyr::case_when(
      logFC_x > 0 & logFC_y > 0 ~ "Exacerbated Up",
      logFC_x < 0 & logFC_y < 0 ~ "Exacerbated Down",
      logFC_x > 0 & logFC_y < 0 ~ "Reversed (Dis Up / Re Down)",
      TRUE                       ~ "Reversed (Dis Down / Re Up)"))
...
message(sprintf("  F04 panel A: %d proteins | %d DEPs | rho = %.3f [%.3f, %.3f] | %.0f%% reversed",
                n_total_A, n_sig_A, r_spear_A, rho_lo_A, rho_hi_A, pct_rev_A))
...
run_set_ora <- function(quad_name) {
  ...
  res |>
    ...
    dplyr::arrange(dplyr::desc(neg_log10_padj)) |> dplyr::slice_head(n = N_SHOW)
}
ora_q1 <- run_set_ora("Exacerbated Up")
ora_q2 <- run_set_ora("Reversed (Dis Down / Re Up)")
ora_q3 <- run_set_ora("Exacerbated Down")
ora_q4 <- run_set_ora("Reversed (Dis Up / Re Down)")
all_quad_ora <- dplyr::bind_rows(
  dplyr::mutate(ora_q1, set = "Exacerbated Up"),
  dplyr::mutate(ora_q2, set = "Reversed (Dis Down / Re Up)"),
  dplyr::mutate(ora_q3, set = "Exacerbated Down"),
  dplyr::mutate(ora_q4, set = "Reversed (Dis Up / Re Down)"))
...
sig_alpha <- ifelse(sig_plot$sig_class == "Sig Both", 0.85, 0.9)

q_tab <- scatter_df |>
  dplyr::count(quadrant, name = "n_total") |>
  dplyr::left_join(scatter_df |> dplyr::filter(is_sig) |> dplyr::count(quadrant, name = "n_sig"), by = "quadrant") |>
...
p_scatter <- ggplot(mapping = aes(x = logFC_x, y = logFC_y)) +
  annotate("rect", xmin = 0, xmax = Inf,  ymin = 0, ymax = Inf,  fill = "#FFE0E0", alpha = 0.55, color = "grey70", linewidth = 0.2) +
  annotate("rect", xmin = -Inf, xmax = 0, ymin = -Inf, ymax = 0,  fill = "#FFE0E0", alpha = 0.55, color = "grey70", linewidth = 0.2) +
  annotate("rect", xmin = 0, xmax = Inf,  ymin = -Inf, ymax = 0,  fill = "#DCEEFF", alpha = 0.55, color = "grey70", linewidth = 0.2) +
  annotate("rect", xmin = -Inf, xmax = 0, ymin = 0, ymax = Inf,  fill = "#DCEEFF", alpha = 0.55, color = "grey70", linewidth = 0.2) +
  ...
  geom_point(data = ns_df, color = "grey80", fill = "grey85", shape = 21, size = 0.7, alpha = 0.30, stroke = 0.12) +
  ...
  annotate("label", x = lim_x, y = lim_y, label = sprintf("Exacerbated Up\n%d/%d", qn("Exacerbated Up","n_sig"), qn("Exacerbated Up","n_total")),
           hjust = 1, vjust = 1, size = txt_quad, fontface = "bold", color = COMP_RED, fill = alpha("white", 0.92), label.padding = unit(2.5, "pt"), lineheight = 0.9) +
  annotate("label", x = -lim_x, y = -lim_y, label = sprintf("%d/%d\nExacerbated Down", qn("Exacerbated Down","n_sig"), qn("Exacerbated Down","n_total")),
           hjust = 0, vjust = 0, size = txt_quad, fontface = "bold", color = COMP_RED, fill = alpha("white", 0.92), label.padding = unit(2.5, "pt"), lineheight = 0.9) +
  annotate("label", x = -lim_x, y = lim_y, label = sprintf("Reversed (Dis↓ Re↑)\n%d/%d", qn("Reversed (Dis Down / Re Up)","n_sig"), qn("Reversed (Dis Down / Re Up)","n_total")),
           hjust = 0, vjust = 1, size = txt_quad, fontface = "bold", color = COMP_BLUE, fill = alpha("white", 0.92), label.padding = unit(2.5, "pt"), lineheight = 0.9) +
  annotate("label", x = lim_x, y = -lim_y, label = sprintf("%d/%d\nReversed (Dis↑ Re↓)", qn("Reversed (Dis Up / Re Down)","n_sig"), qn("Reversed (Dis Up / Re Down)","n_total")),
           hjust = 1, vjust = 0, size = txt_quad, fontface = "bold", color = COMP_BLUE, fill = alpha("white", 0.92), label.padding = unit(2.5, "pt"), lineheight = 0.9) +
  ...
  annotate("text", x = 0, y = -lim_y * 0.97, vjust = 0, hjust = 0.5,
           label = "log₂FC  Disease_Phe (PHE − Ctl)",
...
p_ul <- make_half_bars(ora_q2, COMP_BLUE, "left",  c(0, 2.8))    # Reversed Dis↓Re↑
p_ll <- make_half_bars(ora_q3, COMP_RED,  "left",  c(-2.8, 0))   # Exacerbated Down
p_ur <- make_half_bars(ora_q1, COMP_RED,  "right", c(0, 2.8))    # Exacerbated Up
p_lr <- make_half_bars(ora_q4, COMP_BLUE, "right", c(-2.8, 0))   # Reversed Dis↑Re↓

# Significance key (mito = bold outline, explained in the panel subtitle) --
key_lvls    <- c("Sig Both", "Sig Disease only", "Sig Rescue only")
key_display <- c("Sig Both", "Sig Dis.", "Sig Res.")
key_df <- tibble::tibble(category = factor(key_lvls, levels = key_lvls), display = key_display,
                         fill_col = unname(SIG_COLORS_F3[key_lvls]), x = c(1.4, 2.1, 2.85), y = 0)
p_key <- ggplot(key_df, aes(x = x, y = y)) +
  ...
  scale_x_continuous(limits = c(0.2, 5.3), expand = c(0, 0)) +
