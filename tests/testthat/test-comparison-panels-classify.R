...
test_that("classify_quadrant assigns signed quadrants and gates on significance", {
  comb <- tibble::tibble(
    uniprot_id = c("P1", "P2", "P3", "P4", "P5"),
    gene = c("Ga", "Gb", "Gc", "Gd", "Ge"),
    logFC_CTLvPHE = c(1, -1, 1, -1, 0.9),
    logFC_PHEvPHE_MITO = c(-1, 1, 1, -1, -0.8),
    pi_score_CTLvPHE = c(0.01, 0.01, 0.01, 0.01, 0.20),
    pi_score_PHEvPHE_MITO = c(0.01, 0.01, 0.01, 0.01, 0.20)
  )
