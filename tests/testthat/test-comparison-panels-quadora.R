...
test_that("build_quadrant_ora tolerates a duplicated background gene symbol", {
  set_a <- paste0("Gene", 1:20) # a fake pathway
  bg <- c(paste0("Bg", 1:300), "Bg1") # Bg1 repeated, universe non-unique
  comb <- tibble::tibble(
    uniprot_id = c(set_a, bg), gene = c(set_a, bg),
    logFC_CTLvPHE = c(rep(1, 20), rep(0, 301)),
    logFC_PHEvPHE_MITO = c(rep(-1, 20), rep(0, 301)),
    pi_score_CTLvPHE = c(rep(0.001, 20), rep(0.9, 301)),
    pi_score_PHEvPHE_MITO = c(rep(0.001, 20), rep(0.9, 301))
  )
  pw <- list(FAKE_SET = set_a)
  out <- build_quadrant_ora(
    comb, "CTLvPHE", "PHEvPHE_MITO",
